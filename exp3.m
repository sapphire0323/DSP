...
for i=1:1001
    for k=1:101
        DTFT_R1(i)=DTFT_R1(i)+f_t1(k)*exp(-1i*w(i)*(k-51));
    end
end
...
for i=1:1001
    for k=1:101
        DTFT_R2(i)=DTFT_R2(i)+f_t2(k)*exp(-1i*w(i)*(k-51));
    end
end
...
n=0:100;
k=0:100;
DFT_w1=f_t1*(exp(-1i*2*pi/101)).^(n'*k);
subplot(3,2,3);
stem(k,abs(DFT_w1));
title('DFT R1');

DFT_w2=f_t2*(exp(-1i*2*pi/101)).^(n'*k);
subplot(3,2,4);
stem(k,abs(DFT_w2));
title('DFT R2');
...
zero=zeros(1,202);
f_t3=[f_t1,zero];
DFT_w3=f_t3*(exp(-1i*2*pi/303)).^(n'*k);
subplot(3,2,5);
stem(k,abs(DFT_w3));
title('DFT R1(延长)');

f_t4=[f_t2,zero];
DFT_w4=f_t4*(exp(-1i*2*pi/303)).^(n'*k);
subplot(3,2,6);
stem(k,abs(DFT_w4));
title('DFT R2(延长)');